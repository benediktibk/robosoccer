...
\begin{tikzpicture}
	...
	\begin{pgfonlayer}{background}
		...
		\draw [very thick, color=black!60!white, rounded corners=1.5cm, name path=goalline]  (0,-5) -- (0, 0) -- (0, 5);
...
%	\draw[color=red, line width=.7cm, opacity=.5] (.35, 1.3) -- (.35, -1.3);
	\draw[dashed, thick, name path=robot, opacity=0] (.35, 3) -- (.35, 0);
	...
	\draw[thick, dashed, name path=ball2] (ballpos2) -- ($ (ballpos2)!8cm!(direction2) $);
	\ball at (1.5,5) {};

	\path [name intersections={of=robot and ball2,by=x2}];
	\path [name intersections={of=goalline and ball2,by=x3}];
	
	\draw [very thick, densely dashed, red] (x2) -- ($(x2)!1cm!-130:(direction2)$);
	
	\draw [thick, red] (x3) ++(0,-1.5) arc (-90:-105:1.5) (x3) ++(-96:1.5) node [right, rotate=-6] {$\alpha < 30^\circ$};
