...
protected:
	...
	void UpdateBallInfo(void)
	{
//		do
//		{
		while(mCam.getBallPosition(mBall));
//		} while((mBall.pos_x == 5000.0)||(mLastBallTimestamp >= mBall.timestamp));
...
//		mLastBallTimestamp = mBall.timestamp;
	 ...
	 */
	double GetX(void) 		{this->UpdateBallInfo();return mBall.pos_x;}
	/** Get the current y-coordinate of the ball.
	 * @return Y-coordinate in m.
	 */
	double GetY(void) 		{this->UpdateBallInfo();return mBall.pos_y;}
	/** Get the current moving direction of the ball.
	 * @return Moving direction as an Angle object.
	 */
	Angle GetPhi(void) 		{this->UpdateBallInfo();return Angle(mBall.phi);}
	/** Get the current velocity of the ball.
	 * @return Velocity in m/s.
	 */
	double GetVelocity(void)	{this->UpdateBallInfo();return mBall.velocity;}
