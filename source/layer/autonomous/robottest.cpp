...
void RobotTest::goTo_alreadyAtPosition_noCallToMoveRobot()
{
	...
	m_robot->goTo(m_targets, DriveMoveDefault);
	m_robot->update();
...
void RobotTest::update_goToAndInitialRotationNotReachedAndObstacleMovedALittleBit_turningToSecondPoint()
{
	...
	obstacles.push_back(Circle(Point(1,1),0.5));
	m_targets.push_back(Pose(Point(5, 4), Angle::getQuarterRotation()));
	m_robotWithRealRouter->goTo(m_targets, DriveMoveDefault);
...
void RobotTest::update_goToAndObstacleOnTargetAndIgnoreObstacle_robotGotCallToDrive()
{
	...
	m_targets.push_back(Pose(Point(5, 4), Angle::getQuarterRotation()));
	m_robotWithRealRouter->goTo(m_targets, DriveModeIgnoreGoalObstacles);
...
{
	m_targets.push_back(Pose(Point(1, 3), Angle::getEighthRotation()));
	m_robot->goTo(m_targets, DriveMoveDefault);

	m_robot->update();
	m_hardwareRobot->setPose(Pose(Point(0, 0), Angle(Point(0, 0), Point(1, 3))));
	m_robot->update();
	m_hardwareRobot->setPose(m_targets.front());
	m_robot->update();
...
{
	m_targets.push_back(Pose(Point(1, 3), Angle::getHalfRotation()));
	m_robot->goTo(m_targets, DriveMoveDefault);
...
void RobotTest::goTo_movementStoppedBefore_movementNotStopped()
{
	...
	m_targets.push_back(Pose(Point(1, 2), Angle(3)));
	m_robot->goTo(m_targets, DriveMoveDefault);
